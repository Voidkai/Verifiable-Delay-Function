...
func iterateSquarings(x *ClassGroup, powersToCalculate []int, stop <-chan struct{}) map[int]*ClassGroup {
	powersCalculated := make(map[int]*ClassGroup)

	previousPower := 0
	currX := CloneClassGroup(x)
	sort.Ints(powersToCalculate)
	for _, currentPower := range powersToCalculate {

		for i := 0; i < currentPower-previousPower; i++ {
			currX = currX.Pow(2)
			if currX == nil {
				return nil
			}
		...
		powersCalculated[currentPower] = currX
...
func GenerateVDF(seed []byte, iterations, intSizeBits int) ([]byte, []byte) {
	return GenerateVDFWithStopChan(seed, iterations, intSizeBits, nil)
}

func GenerateVDFWithStopChan(seed []byte, iterations, intSizeBits int, stop <-chan struct{}) ([]byte, []byte) {
	defer timeTrack(time.Now())

	D := CreateDiscriminant(seed, intSizeBits)
	x := NewClassGroupFromAbDiscriminant(big.NewInt(2), big.NewInt(1), D)

	y, proof := calculateVDF(D, x, iterations, intSizeBits, stop)
...
func VerifyVDF(seed, proofBlob []byte, iterations, intSizeBits int) bool {
	defer timeTrack(time.Now())
	...
	D := CreateDiscriminant(seed, intSizeBits)
	x := NewClassGroupFromAbDiscriminant(big.NewInt(2), big.NewInt(1), D)
	y, _ := NewClassGroupFromBytesDiscriminant(proofBlob[:(2*intSize)], D)
	proof, _ := NewClassGroupFromBytesDiscriminant(proofBlob[2*intSize:], D)
...
//Optimized evaluation of h ^ (2^T // B)
func evalOptimized(identity, h *ClassGroup, B *big.Int, T, k, l int, C map[int]*ClassGroup) *ClassGroup {
	...
	for j := l - 1; j > -1; j-- {
		//x = pow(x, pow(2, k))
		bLimit := int64(math.Pow(2, float64(k)))
		x = x.Pow(bLimit)
		if x == nil {
			return nil
		}
		...
		//ys = {}
		ys := make([]*ClassGroup, bLimit)
		for b := int64(0); b < bLimit; b++ {
			ys[b] = identity
		}
...
func calculateVDF(discriminant *big.Int, x *ClassGroup, iterations, int_size_bits int, stop <-chan struct{}) (y, proof *ClassGroup) {
	...
	powersToCalculate := make([]int, loopCount+2)

	for i := 0; i < loopCount+1; i++ {
		powersToCalculate[i] = i * k * L
	}

	powersToCalculate[loopCount+1] = iterations

	powers := iterateSquarings(x, powersToCalculate, stop)
